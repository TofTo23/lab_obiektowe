...
enum class WeaponType { LASER, BULLET, BLITZ,  COUNT };
class Projectile {
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			Rectangle lr = { transform.position.x - 2.f, transform.position.y - LASER_LENGTH, LASER_LENGTH,  4.f };
			DrawRectangleRec(lr, RED);
		}
		else {
			DrawCircleV(transform.position, 5.f, YELLOW);
		}
	}
	Vector2 GetPosition() const {
		return transform.position;
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("orzel1.png");
...
public:
	...
	void Run() {
		...
		Sound file[3];
		file[0] = LoadSound("strzal1.wav");
		file[1] = LoadSound("strzal2.wav");
		file[2] = LoadSound("s3.wav");
